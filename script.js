/* ===================================================
   ...
   MXL based endorsement, WTP style benefits hook,
   sensitivity, Copilot integration and exports
   =================================================== */

/* ===========================
   Global app and model versions
   =========================== */
...
const COST_TEMPLATES = {
  frontline: {
    combined: {
      ...
      label: "Frontline combined template",
      description: "Frontline cost structure with harmonised components and indirect plus opportunity cost.",
      oppRate: 1.09,
      components: [
        { id: "staff_core", label: "Programme staff salaries and benefits", directShare: 0.214 },
        { id: "office_equipment", label: "Office equipment", directShare: 0.004 },
        { id: "office_software", label: "Office software", directShare: 0.0004 },
        { id: "rent_utilities", label: "Rent and utilities", directShare: 0.024 },
        ...
        { id: "travel_in_country", label: "In country travel", directShare: 0.65 }
      ]
    }
  },
  intermediate: {
    combined: {
      ...
      label: "Intermediate combined template",
      description: "Intermediate cost structure with harmonised components and indirect plus opportunity cost.",
      oppRate: 0.35,
      components: [
        { id: "staff_core", label: "Programme staff salaries and benefits", directShare: 0.0924 },
        { id: "staff_other", label: "Consultants and advisors", directShare: 0.0004 },
        { id: "office_equipment", label: "Office equipment", directShare: 0.0064 },
        { id: "office_software", label: "Office software", directShare: 0.027 },
        { id: "rent_utilities", label: "Rent and utilities", directShare: 0.0171 },
        ...
        { id: "travel_in_country", label: "In country travel", directShare: 0.57 },
        { id: "travel_international", label: "International travel", directShare: 0.1299 },
  ...
  advanced: {
    combined: {
      ...
      label: "Advanced combined template",
      description: "Advanced cost structure with harmonised components and indirect plus opportunity cost.",
      oppRate: 0.30,
      components: [
        { id: "staff_core", label: "Programme staff salaries and benefits", directShare: 0.165 },
        { id: "office_equipment", label: "Office equipment", directShare: 0.0139 },
        { id: "office_software", label: "Office software", directShare: 0.0184 },
        { id: "rent_utilities", label: "Rent and utilities", directShare: 0.0255 },
        { id: "trainee_allowances", label: "Trainee allowances and scholarships", directShare: 0.0865 },
        { id: "trainee_equipment", label: "Trainee equipment", directShare: 0.0035 },
        ...
        { id: "travel_in_country", label: "In country travel", directShare: 0.372 },
        { id: "travel_international", label: "International travel", directShare: 0.288 },
...
(function () {
  ...
  const DEFAULT_ASSUMPTION_SETS = {
    "mohfw-baseline": {
      ...
      note: "Reference set aligned with indicative baseline assumptions."
    },
    "wb-downside": {
      id: "wb-downside",
  ...
  const DEFAULT_CONFIG = {
    ...
    numberOfCohorts: 4,
    planningHorizonYears: DEFAULT_EPI_SETTINGS.general.planningHorizonYears,
    tier: "frontline",
  ...
  function ensureGlobalTooltip() {
    ...
    if (!globalTooltipEl) {
      ...
      globalTooltipEl.innerHTML =
        '<div class="tooltip-title"></div><div class="tooltip-body"></div>';
  ...
  /* ===========================
     MXL based endorsement helper
     =========================== */

  function computeEndorsementFromConfig(config) {
    const tier = config.tier || "frontline";
    const career = config.careerIncentive || "certificate";
    const mentorship = config.mentorship || "low";
    const delivery = config.deliveryMode || "blended";
    const responseDays = String(config.responseDays || 7);

    let utilProgram =
      MXL_COEFS.ascProgram +
      (MXL_COEFS.tier[tier] || 0) +
      (MXL_COEFS.career[career] || 0) +
      (MXL_COEFS.mentorship[mentorship] || 0) +
      (MXL_COEFS.delivery[delivery] || 0) +
      (MXL_COEFS.response[responseDays] || 0);
    ...
    if (!Number.isFinite(denom) || denom <= 0) {
      return clamp(config.endorsementRate || 0.8, 0, 1);
    }
    const pProgram = expProg / denom;
    return clamp(pProgram, 0, 1);
  }
  ...
  function readConfigFromUI() {
    ...
    const map = [
      ...
      {
        id: "#input-trainees-per-cohort",
        alt: "#inputTraineesPerCohort",
        field: "traineesPerCohort",
        numeric: true,
        min: 1,
        max: 500
      },
      {
        id: "#input-number-of-cohorts",
        alt: "#inputNumberOfCohorts",
      ...
      {
        id: "#input-planning-horizon",
        alt: "#inputPlanningHorizon",
      ...
      {
        id: "#input-endorsement-rate",
        alt: "#inputEndorsementRate",
        field: "endorsementRate",
        numeric: true,
        min: 0,
        max: 1
      },
      {
        id: "#input-completion-rate",
        alt: "#inputCompletionRate",
      ...
      {
        id: "#input-career-incentive",
        alt: "#inputCareerIncentive",
        field: "careerIncentive"
      },
      { id: "#input-mentorship", alt: "#inputMentorship", field: "mentorship" },
      { id: "#input-delivery-mode", alt: "#inputDeliveryMode", field: "deliveryMode" }
    ];
    ...
    const responseSelect =
      safeQuerySelector("#input-response-days") ||
      safeQuerySelector("#inputResponseDays");
    if (responseSelect) {
      const v = parseInt(responseSelect.value, 10);
      cfg.responseDays = Number.isFinite(v) ? v : 7;
      // enforce 7 days as fixed outcome in UI
    ...
    if (!cfg.scenarioId) {
      ...
      const idEl =
        safeQuerySelector("#input-scenario-id") ||
        safeQuerySelector("#inputScenarioId");
  ...
  function writeConfigToUI(cfg) {
    ...
    const map = [
      ...
      { id: "#input-tier", alt: "#inputTier", field: "tier" },
      { id: "#input-career-incentive", alt: "#inputCareerIncentive", field: "careerIncentive" },
      { id: "#input-mentorship", alt: "#inputMentorship", field: "mentorship" },
      { id: "#input-delivery-mode", alt: "#inputDeliveryMode", field: "deliveryMode" }
    ];
    map.forEach(function (m) {
  ...
  function readAssumptionsFromUI() {
    ...
    const base =
      STATE.assumptionSets[currentId] ||
      DEFAULT_ASSUMPTION_SETS["mohfw-baseline"];
    const assumptions = { ...base };

    const discountEl =
      safeQuerySelector("#settings-discount-rate") ||
      safeQuerySelector("#settingsDiscountRate");
    const outbreakEl =
      safeQuerySelector("#settings-outbreak-value") ||
      safeQuerySelector("#settingsOutbreakValue");
    const oppEl =
      safeQuerySelector("#settings-opp-cost-share") ||
      safeQuerySelector("#settingsOppCostShare");
    ...
    if (outbreakEl) {
      assumptions.outbreakValue = clamp(outbreakEl.value, 0, 5000000000);
  ...
  function writeAssumptionsToUI(assumptions) {
    ...
    const oppEl =
      ...
      safeQuerySelector("#settingsOppCostShare");
    const presetSelect =
      safeQuerySelector("#settings-assumption-preset") ||
      safeQuerySelector("#settingsAssumptionPreset");
  ...
  function initSettingsForm() {
    ...
    if (presetSelect) {
      ...
      presetSelect.addEventListener("change", function () {
        ...
        if (STATE.assumptionSets[id]) {
          ...
          appendSettingsLog(
            "Switched assumption set to " + STATE.assumptionSets[id].label + "."
          );
    ...
    const applyBtn =
      safeQuerySelector("#btn-apply-settings") ||
      safeQuerySelector("#btnApplySettings") ||
      safeQuerySelector("#adv-apply-settings");
    if (applyBtn) {
      applyBtn.addEventListener("click", function () {
        const assumptions = readAssumptionsFromUI();
        recalculateAndRender();
        appendSettingsLog(
          'Updated assumption values for set "' +
            (assumptions.label || assumptions.id) +
            '".'
        );
  ...
  function appendSettingsLog(message) {
    const logArea =
      safeQuerySelector("#settings-log") ||
      safeQuerySelector("#settingsLog") ||
      safeQuerySelector("#adv-settings-log");
  ...
  function computeScenarioIndicators(config, assumptions) {
    ...
    // Endorsement from MXL
    const endorsementRate = computeEndorsementFromConfig(config);
    ...
    const baseOutbreakResponsesPerGraduatePerYear =
      DEFAULT_EPI_SETTINGS.tiers.frontline.outbreaksPerGraduatePerYear;

    const responseDays = config.responseDays || 7;
    const responseKey = String(responseDays);
    const scaleResponse =
      RESPONSE_TIME_MULTIPLIERS[responseKey] != null
        ? RESPONSE_TIME_MULTIPLIERS[responseKey]
        : 1.0;

    const outbreakResponsesPerYear =
      graduatesEndorsed *
      baseOutbreakResponsesPerGraduatePerYear *
      scaleResponse;
    const outbreakResponsesPerCohortPerYear =
      outbreakResponsesPerYear / Math.max(1, numberOfCohorts);

    // Simple calibration: use annual cost per trainee based on tier duration
    const tier = config.tier || "frontline";
    const months = TIER_MONTHS[tier] || 12;
    const costPerTraineePerMonth = 400000 / months; // can be replaced with Excel calibrated value
    const annualCostPerTrainee = costPerTraineePerMonth * (months / 12);
    const costFlow = annualCostPerTrainee * traineesPerYear;

    const oppShare = assumptions.opportunityCostShare;
    const directCostsPV = presentValueAnnuity(
      costFlow,
    ...
    const totalBenefitPV = presentValueAnnuity(
      benefitFlow,
      assumptions.discountRate,
      horizon
    );
  ...
  function renderHeadlineIndicators(indicators) {
    setText(
      ["#summary-endorsement", "#summaryEndorsement"],
      formatPercent(indicators.endorsementRate, 1)
    );
    setText(
      ["#summary-bcr", "#summaryBcr"],
      indicators.bcr != null ? formatNumber(indicators.bcr, 2) : "n/a"
    );
    setText(
      ["#summary-npv", "#summaryNpv"],
      formatNumber(indicators.npv, 0)
    );
    setText(
      ["#summary-trainees", "#summaryTrainees"],
      formatNumber(indicators.totalTrainees, 0)
    );
    setText(
      ["#summary-graduates", "#summaryGraduates"],
      formatNumber(indicators.graduatesTotal, 0)
    );
    setText(
      ["#summary-outbreaks-per-year", "#summaryOutbreaksPerYear"],
      formatNumber(indicators.outbreakResponsesPerYear, 1)
    );
    setText(
      ["#summary-outbreaks-per-cohort", "#summaryOutbreaksPerCohort"],
      formatNumber(indicators.outbreakResponsesPerCohortPerYear, 2)
    );

    const hintBcr =
      safeQuerySelector("#hint-bcr") || safeQuerySelector("#hintBcr");
    if (hintBcr) {
      if (indicators.bcr != null && indicators.bcr > 1) {
        hintBcr.textContent =
          "Value above 1 suggests benefits exceed economic costs.";
      } else if (indicators.bcr != null) {
        hintBcr.textContent =
          "Value below 1 suggests costs exceed measured monetary benefits.";
      } else {
        hintBcr.textContent =
          "Benefit cost ratio is not available for this configuration.";
      }
    }

    const hintEndorsement =
      safeQuerySelector("#hint-endorsement") ||
      safeQuerySelector("#hintEndorsement");
    if (hintEndorsement) {
      if (indicators.endorsementRate >= 0.7) {
        hintEndorsement.textContent =
          "Above 70 percent typically indicates strong support in the preference study context.";
      } else if (indicators.endorsementRate >= 0.5) {
        hintEndorsement.textContent =
          "Between 50 and 70 percent indicates moderate endorsement in the preference study context.";
      } else {
        hintEndorsement.textContent =
          "Below 50 percent suggests limited endorsement and potential acceptability concerns.";
      }
    }
  }
  ...
  function renderConfigSummary(config, indicators, assumptions) {
    setText(
      [
        "#summary-config-trainees-per-cohort",
        "#summaryConfigTraineesPerCohort"
      ],
      formatNumber(config.traineesPerCohort, 0)
    );
    setText(
      [
        "#summary-config-number-of-cohorts",
        "#summaryConfigNumberOfCohorts"
      ],
      formatNumber(config.numberOfCohorts, 0)
    );
    setText(
      ["#summary-config-horizon-years", "#summaryConfigHorizonYears"],
      formatNumber(config.planningHorizonYears, 0)
    );
    setText(
      ["#summary-config-endorsement", "#summaryConfigEndorsement"],
      formatPercent(indicators.endorsementRate, 1)
    );
    setText(
      ["#summary-config-completion", "#summaryConfigCompletion"],
      formatPercent(config.completionRate, 1)
    );

    const assumptionEl =
      safeQuerySelector("#summary-assumption-label") ||
      safeQuerySelector("#summaryAssumptionLabel");
    if (assumptionEl && assumptions) {
      assumptionEl.textContent = assumptions.label || assumptions.id;
    }

    setText(
      ["#summary-cost-direct", "#summaryCostDirect"],
      formatNumber(indicators.directCostsPV, 0)
    );
    setText(
      ["#summary-cost-opportunity", "#summaryCostOpportunity"],
      formatNumber(indicators.opportunityCostPV, 0)
    );
    setText(
      ["#summary-cost-total", "#summaryCostTotal"],
      formatNumber(indicators.totalCostPV, 0)
    );
    setText(
      ["#summary-benefit-pv", "#summaryBenefitPv"],
  ...
  function renderNationalSummary(config, indicators) {
    setText(
      ["#national-graduates", "#nationalGraduates"],
      formatNumber(indicators.graduatesTotal, 0)
    );
    setText(
      ["#national-outbreaks", "#nationalOutbreaks"],
      formatNumber(
        indicators.outbreakResponsesPerYear * config.planningHorizonYears,
        0
      )
    );
    setText(
      ["#national-label-horizon", "#nationalLabelHorizon"],
      formatNumber(config.planningHorizonYears, 0)
    );
  }
  ...
  function renderScenarioTable() {
    const tableBody =
      safeQuerySelector("#scenario-table tbody") ||
      safeQuerySelector("#scenarioTable tbody");
    if (!tableBody) return;
    ...
    STATE.scenarios.forEach(function (sc, index) {
      ...
      tdBcr.textContent =
        sc.indicators.bcr != null ? formatNumber(sc.indicators.bcr, 2) : "n/a";
      ...
      loadBtn.addEventListener("click", function () {
        ...
        writeAssumptionsToUI(
          STATE.assumptionSets[sc.assumptionSetId] ||
            DEFAULT_ASSUMPTION_SETS["mohfw-baseline"]
        );
        recalculateAndRender();
        showToast(
          'Scenario "' + sc.name + '" loaded into configuration',
          "success"
        );
      });
  ...
  function renderDiagnostics(config, assumptions, indicators) {
    const diagnosticsEl =
      safeQuerySelector("#diagnosticsContent") ||
      safeQuerySelector("#diagnostics-content");
  ...
  function renderSimpleCharts(config, indicators) {
            ...
            datasets: [
              {
                ...
                data: [
                  indicators.outbreakResponsesPerYear,
                  indicators.outbreakResponsesPerCohortPerYear
                ]
              }
            ]
          },
    ...
    setText(
      ["#chart-costs-summary", "#chartCostsSummary"],
    ...
    setText(
      ["#chart-benefits-summary", "#chartBenefitsSummary"],
    ...
    setText(
      ["#chart-outbreaks-summary", "#chartOutbreaksSummary"],
  ...
  function addScenarioFromCurrentConfig() {
    ...
    const existingIndex = STATE.scenarios.findIndex(function (s) {
  ...
  function initScenarioControls() {
    const saveBtn =
      safeQuerySelector("#btn-save-scenario") ||
      safeQuerySelector("#btnSaveScenario");
    const newBtn =
      safeQuerySelector("#btn-new-scenario") ||
      safeQuerySelector("#btnNewScenario");
    ...
    if (newBtn) {
      newBtn.addEventListener("click", function () {
        ...
        writeConfigToUI(STATE.config);
        showToast(
          "New blank scenario created. Adjust inputs and save when ready.",
          "success"
        );
      });
    }
  ...
  function buildCopilotPrompt(config, assumptions, indicators) {
    ...
    const scenarioInputs = {
      tier: config.tier,
      careerIncentive: config.careerIncentive,
  ...
  function renderCopilotPrompt(config, assumptions, indicators) {
    const outputEl =
      safeQuerySelector("#copilot-prompt-output") ||
      safeQuerySelector("#copilotPromptOutput");
    if (!outputEl) return;
  ...
  function initCopilotControls() {
    const generateBtn =
      safeQuerySelector("#btn-generate-copilot") ||
      safeQuerySelector("#btnGenerateCopilot");
    const copyBtn =
      safeQuerySelector("#btn-copy-copilot") ||
      safeQuerySelector("#btnCopyCopilot");
    ...
    if (generateBtn) {
      generateBtn.addEventListener("click", function () {
        ...
        showToast(
          "Copilot prompt prepared. Copy and paste into Microsoft Copilot.",
          "success"
        );
      });
    }
    ...
    if (copyBtn) {
      copyBtn.addEventListener("click", function () {
        ...
        if (!text.trim()) {
          showToast(
            "Nothing to copy. Generate the Copilot prompt first.",
            "warning"
          );
          return;
        }
        if (navigator.clipboard && navigator.clipboard.writeText) {
          ...
          showToast(
            "Clipboard not available. Select the text and copy manually.",
            "warning"
          );
        }
      });
  ...
  function initHelpPanel() {
    ...
    const openBtn =
      safeQuerySelector("#btn-open-help") ||
      safeQuerySelector("#btnOpenHelp");
    const closeBtn =
      safeQuerySelector("#btn-close-help") ||
      safeQuerySelector("#btnCloseHelp");
    ...
    document.addEventListener("keydown", function (event) {
      if (
        event.key === "Escape" &&
        helpPanel.getAttribute("aria-hidden") === "false"
      ) {
        closePanel();
      }
    });
  ...
  function persistState() {
    try {
      ...
      localStorage.setItem(
        ...
        JSON.stringify(STATE.assumptionSets)
      );
      localStorage.setItem(
        STORAGE_KEYS.LAST_STATE,
        JSON.stringify({
          config: STATE.config
        })
      );
  ...
  function initConfigForm() {
    ...
    const applyBtn =
      safeQuerySelector("#btn-apply-values") ||
      safeQuerySelector("#btnApplyValues");
    const resetBtn =
      safeQuerySelector("#btn-reset-config") ||
      safeQuerySelector("#btnResetConfig");
    ...
    const configInputs = safeQuerySelectorAll(
      "#configTab input, #configTab select"
    );
  ...
  function recalculateAndRender() {
    try {
      ...
      showToast(
        "An error occurred while updating the scenario. Please check the console.",
        "error"
      );
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    if (versionValueEl) {
      versionValueEl.textContent =
        "App " + APP_VERSION + " · Model " + MODEL_VERSION;
    }
