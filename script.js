...
function displayWarnings(msgs){
  const w=document.getElementById("warnings");
  if(msgs.length){
    w.innerHTML = "<ul>" + msgs.map(m=>`<li>${m}</li>`).join("") + "</ul>";
...
/* -------- Placeholder Coeffs & WTP -------- */
const COEFS = {
  ASC:0.30, type_intermediate:0.28, type_advanced:0.48,
...
function buildScenario(){
  ...
  if(!sc.ptype||!sc.duration||!sc.focus||!sc.mode||!sc.resp){
    alert("Please select all categorical attributes.");
    return null;
  }
  const warn = [];
...
function buildRecommendation(sc,ePct,bcr,net){
  ...
  if(ePct>=70 && bcr>=1 && net>0){
    msgs.push("Strong scenario: high endorsement with positive net benefit—proceed to detailed planning.");
  }
  if(ePct<45){
    msgs.push("Low endorsement: consider shortening duration to 6–12m, adding in-person mentorship, or improving response capacity.");
  }
  if(bcr<1 || net<0){
    msgs.push("Benefits < costs: trim per-trainee allowances/equipment or reduce fixed overheads.");
  }
  if(sc.mode==="online" && (sc.ptype==="advanced"||sc.focus==="onehealth")){
    msgs.push("Online delivery for Advanced/One Health reduces field exposure—switch to Hybrid or In-person.");
  }
  if(sc.duration==="24"){
    msgs.push("24-month programs are costly: consider modular 12-month core with refreshers.");
  }
  if(sc.resp==="14"){
    msgs.push("Improving response to 7 or 3 days often yields higher WTP at modest extra cost.");
  }
  if(sc.costPerTM>70000){
    msgs.push("High MoH cost per trainee: explore co-funding or state partnerships to reduce burden.");
  }
...
const PUB_COLORS=["#2a76d2","#009688","#f39c12","#e74c3c","#7f8c8d"];

function renderWTPChart(){
  const ctx = document.getElementById("wtpChartMain").getContext("2d");
  if(wtpChart) wtpChart.destroy();
  const labels = Object.keys(WTP).map(k=>k.replace(/_/g," "));
  const data = Object.values(WTP);
  wtpChart = new Chart(ctx, {
    type: "bar",
    data: { labels, datasets:[{label:"Marginal WTP (₹)", data, backgroundColor:PUB_COLORS, borderColor:"#243447", borderWidth:1}]},
    options:{responsive:true,maintainAspectRatio:false,scales:{y:{beginAtZero:true}},plugins:{legend:{display:false},title:{display:true,text:"Marginal WTP (₹)",font:{size:18}}}}
  });
}
...
function renderEndorseChart(sc){
  ...
  endorseChart = new Chart(ctx, {
    ...
    options:{responsive:true,maintainAspectRatio:false,plugins:{title:{display:true,text:`Endorsement: ${p.toFixed(1)}%`,font:{size:18}},tooltip:{callbacks:{label:c=>`${c.label}: ${c.parsed.toFixed(1)}%`}}}}
  });
}

function renderCostsBenefits(sc,ePct,w,c,bcr,net,eShare){
  const cont = document.getElementById("costsBenefitsResults"); if(!cont) return;
  const s=sc||buildScenario(), es=eShare!==null?eShare:endorsementProb(s), e=ePct!==null?ePct:es*100;
  const W=w!==null?w:scenarioWTP(s,es), C=c!==null?c:scenarioCost(s,es), B=bcr!==null?bcr:W/C, N=net!==null?net:W-C;
  ...
  combinedChart = new Chart(ctx, {
    ...
    options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},title:{display:true,text:"Cost–Benefit Summary (₹)",font:{size:18}}},scales:{y:{beginAtZero:true}}}
  });
}

function addCostCard(cont,title,val,type){
  const div=document.createElement("div"); div.className="cost-card";
  div.innerHTML = `<h4>${title.replace(/_/g," ").replace(/\b\w/g,c=>c.toUpperCase())}</h4>
                   <p>${type} component</p><p><strong>₹ ${formatINR(val)}</strong></p>`;
...
function renderQALY(){
  ...
  qalyChart=new Chart(ctx,{
    ...
    options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},title:{display:true,text:"ICER vs Threshold",font:{size:18}}},scales:{y:{beginAtZero:true}}}
  });
}

/* -------- PSA -------- */
let psaBCRChart, psaICERChart;
function runPSA(){
  const sc=buildScenario(); if(!sc) return;
  ...
  for(let i=0;i<iters;i++){
    ...
    BCR.push(w/c); NB.push(w-c); ICER.push(c/q);
  }

  const mean = arr=>arr.reduce((a,b)=>a+b,0)/arr.length;
  const ci = arr=>{ const s=[...arr].sort((a,b)=>a-b), n=s.length; return [s[Math.floor(0.025*n)], s[Math.floor(0.975*n)]]; };
...
function renderHistogram(id, data, label, color){
  ...
  data.forEach(v=>{let i=Math.floor((v-min)/width); if(i>=bins) i=bins-1; counts[i]++;});
  const labels=counts.map((_,i)=>(min+i*width).toFixed(2));
  const cfg={type:"bar",data:{labels,datasets:[{label,data:counts,backgroundColor:color,borderColor:"#243447",borderWidth:1}]},
    options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},title:{display:true,text:`${label} Distribution`,font:{size:16}}},scales:{y:{beginAtZero:true}}}
  };
...
function saveScenario(){
  ...
  const q=scenarioQALY(sc,eShare,qpt), icer=c/q;
...
function appendScenarioRow(s){
  const tbody=document.querySelector("#scenarioTable tbody"), tr=document.createElement("tr");
  ["name","ptype","duration","focus","mode","resp","capacity","costPerTM","stakeholders","cohortsYear","yearsHorizon","discRate","endorse","totalWTP","totalCost","bcr","net","icer"]
    .forEach(c=>{ const td=document.createElement("td"); let v=s[c];
...
function exportPDF(){
  if(!savedScenarios.length){ alert("No scenarios saved."); return; }
  ...
  savedScenarios.forEach(s=>{
    ...
    [
      ...
      `₹/T/M:${formatINR(s.costPerTM)} Stakeh:${s.stakeholders}`,
...
function formatINR(n){ return Math.round(n).toString().replace(/\B(?=(\d{2})+(?!\d))/g,","); }
